...
class Agent(nn.Module):
    def __init__(self, envs):
        ...
        self.network = nn.Sequential(
            nn.Linear(gym.spaces.flatdim(envs.single_observation_space), 256),
            nn.ReLU(),
            nn.Linear(256, 256),
            nn.ReLU()
        )
    ...
    def get_pi(self, x):
        return Categorical(logits=self.actor(self.network(x)))
...
if __name__ == "__main__":
    ...
    envs = gym.vector.make(args.env_id, num_envs=args.num_envs, asynchronous=False, wrappers=[gym.wrappers.FlattenObservation])
