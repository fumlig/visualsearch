...
This gives us control over the difficulty of the environments, and the number of training and test samples.
...
\begin{align}
    ...
    p_p & \in \{0, \dots, 9\} \times \{0, \dots, 9\}
\end{align}

The agent always moves in a \(10 \times 10\) grid.
...
With this in mind we design our neural network architecture as follows:
...
Here, \(z_t \in \mathbb{R}^{64 \times 10 \times 10}\) is a feature map with one feature vector per possible position \(p_t\).
...
This means that they can make use of (a) visual features of previously explored locations, (b) spatial relationships between these features, and (c) the current position of the agent.
Finally, the feature map is updated so that \(z_{t+1}^{(p_t)} = \omega_{t}\).
...
\subsection{Impact of Search Space Size}

\subsection{Generalization with Limited Samples}
