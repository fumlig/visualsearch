...
Some other works reward actions that move towards targets and penalize those that move away from targets, using the supervised distance delta. % cite
...
\begin{itemize}
    ...
    \item the current position of the agent \(p_t\). % call this something else, what is the internal state of the sensor called?
    % previous action?
    % the explicit memory can be built exclusively from these two
\end{itemize}
