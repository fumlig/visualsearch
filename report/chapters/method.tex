...
Let the task be \(\left\langle \mathcal{M}, \mathcal{T}_0 \right\rangle\), where \(\mathcal{M}\) is a POMDP and \(\mathcal{T}_0\) is the probability distribution on the initial states.
...
The initial states are drawn from \(\mathcal{T}_0(p)\), where \(p\) is a \textit{random seed}.
The seed determines the appearance of the scene, the location of the targets and the initial position of the agent.

Furthermore, the appearance of the scenes and the location of targets have some correlation in all environments.
...
The \(\mathtt{TRIGGER}\) is used to indicate that a target is in view.

% https://www.reddit.com/r/reinforcementlearning/comments/ggkhwq/d_does_a_constant_penalty_incite_the_agent_to/
...
% environment 3: hard, realistic, zoom?
...
Furthermore, we reduce the number of steps per rollout to be much less than the episode length.
