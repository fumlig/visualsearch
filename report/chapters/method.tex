...
We can now define the problem tackled formally.
...
% this might be better in introduction
...
The second reward signal, similar to that of \cite{caicedo_active_2015,...}, is defined as
...
\(R_2\) uses the supervised distance between targets and the agent which is available during training time. % cite
...
\begin{itemize}
    ...
    \item the current position of the agent \(p_t \in  \mathbb{R}^{W \times H}\). % call this something else, what is the internal state of the sensor called?
\end{itemize}
...
Terminating episodes early this way is common to speed up training. % cite
