...
Furthermore, we use the same parameters as those outlines in ~\cite{cobbe_procgen_2020}.
These are presented in Table~\ref{tab:hyperparameters}.
We find that using many parallell environments stabilizes the training.
...
\begin{table}
    ...
    \begin{tabular}{lcc}
        ...
        \(\gamma\) & .999 \\
        \(\lambda\) & .95 \\
        \(\text{timesteps per rollout}\) & 256 \\
        \(\text{epochs per rollout}\) & 3 \\
        \(\text{minibatches per epoch}\) & 8 \\
        \(\text{entropy bonus}\) & 0.01 \\
        \(\text{clip range}\) & .2 \\
        \(\text{reward normalization}\) & yes \\
        \(\text{learning rate}\) & \(5 \times 10^{-4}\) \\
        \(\text{parallell environments}\) & 64 \\
        \(\text{total timesteps}\) & \(25 \times 10^6\) \\
        \bottomrule
    \end{tabular}
...
We train the baselines with the same algorithm as our approach.
We also use the same hyperparameters.
