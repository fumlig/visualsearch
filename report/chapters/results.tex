...
In both the gaussian environment and terrain environment, there is a total of 100 positions in the search space, and in the camera environment there is a total of 200 positions.
This means that the random and greedy baselines take many redundant steps that revisit already explored positions.
...
Notably, the search paths are particularly short in the camera environment.
This is related to the fact that targets are most commonly found at the horizon, and the deterministic ordering of actions happened to favor that distribution.
...
Overall, the spatial memory agent achieves the most competitive search path lengths and SPL scores.
The temporal memory agent struggles with avoiding revisits in the first two environments.
In the camera environment, both of our approaches achieve similar average length.
