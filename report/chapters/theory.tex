...
%Sections \ref{sec:visualsearch} and \ref{sec:activevision} provide perspective from neuroscience and robotics respectively.
...
\subsection{Active Vision and Perception} 
...
Although looking for targets in an unknown environment is inherently an active vision task,
there is an argument to be made that active vision is a means to solve the task more effectively.
Indeed, active vision systems have several computational advantages over passive ones~\cite{aloimonos_active_1988}.
Furthermore, an active learning system can select more informative samples to learn from than a passive one.

% Samples that have already been learnt from risk being overused,
% and there might be samples that an incremental on-policy algorithm can not yet understand.
...
it would seem like a fitting framework for learning to search.
...
In this section, we survey some of the more relevant ones.
Although there is significant overlap between the fields, 
we organize the works into subsections based on their targeted task.

\subsection{Object Search}

In robotics, the task of searching for objects in unknown environments is sometimes referred to as \textit{object search}.
Several tasks, like searching for objects, require intentional actions.
...
% https://www.semanticscholar.org/paper/A-Complexity%E2%80%90Level-Analysis-of-the-Sensor-Planning-Ye-Tsotsos/cab92e1c6aff529f536bf068299d72a0323f9f21?sort=total-citations

Suppose that an agent is to search a region for a single object with a controllable camera in three dimensions.
The state of the searcher is uniquely determined by the position of the camera.
Actions adjust the camera configuration at a certain cost.
After each action the agent observes the searched region through the camera, and detects objects.
The agent has some knowledge of the target probability distribution.
Planning a sequence of actions that maximize the probability of finding the target, under some cost constraint, is proven to be NP-complete.~\cite{ye_tsotsos_2001}
Approximate solutions are necessary.
% Even when using a greedy solution, it is difficult to learn probabilities and how they change from observations in the general case.

Forssén et al~\cite{forssen_visual_2008} implement a mobile robot that searches for specific objects within a cluttered environment\dots
...
\cite{aydemir_real_2011}\dots

\cite{aydemir_active_2013} distinguish between\dots


% Pretty sure I had more of these?

\todo{More emphasis here. Particularly on \cite{shubina_visualsearch_2010} and related works which seem especially relevant.}
...
%Krishna et al.~\cite{krishna_tetromino_2020}...

\subsection{Visual Navigation}

\todo{Compress some of these. Similar works can be presented together.}
...
\todo{Many of these assume that objects are discrete and recognizable. What is there are colors or patterns instead?}
...
Oh et al. ~\cite{oh_minecraft_2016} propose one such memory which is differentiable.
The memory contains a mechanism for retrieving memory contents based on the current context of the agent\dots
A similar memory architecture is introduced by Parisotto and Salakhutdinov~\cite{parisotto_salakhutdinov_2017}\dots
...
They conclude that the an agent's policy should rely only on features which will behave similarly in the training and testing context.
If the similarities between training and testing contexts are known,
they can be encoded as inductive bias for stronger generalization.
