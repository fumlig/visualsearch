...
In active vision, searching for objects with a camera in unknown environments is referred to as \textit{object search}.
...
Finding an optimal solution in three dimensions has been shown to be NP-complete, necessitating approximate solutions~\cite{ye_tsotsos_2001,andreopoulos_tsotsos_theory_2009}.

\todo{verify np-complete/np-hard claims using second source.}
...
\todo{Find a better way to organize these. There is significant overlap between the tasks.}
...
\todo{All of these are for navigation.}
...
\todo{Should get into why procedural generation is and is not a good idea for RL. It is discussed in \cite{kirk_survey_2022}.}
...
The authors propose the following set of metrics that better reflect performance across a handful of runs:

\todo{Rework this -- they are a bit more nuanced than this}.
%However, only increasing the number of runs to reduce statistical uncertainty is not feasible.
