...
\subsection{Visual Search, Visual Attention and Active Vision}
\label{sec:visualsearch}

% top-down attention is the one we are after

% avoid referring to the task as visual search?
% simply mention that this is what it is called in neuroscience

% covert/overt: reactionary/requires memory!

% should merge all these three maybe?
% visual search and attention are closely intertwined

% todo: should maybe remove the psychology and keep that in introduction only


% computational systems may benefit from foveated vision,
% but hardware is currently not build that way.
% we could model it in software but we are more focused on directing gaze
...
Something that is crucial for visual 
...
\todo{\cite{nakayama_situating_2011} is good, discusses detection, memory, reinforcement, etc.
Section 7 contains good material}.
