...
Animal perception, in contrast, is active - we do not only see things, but look for them.
Aloimonos et al.~\cite{aloimonos_active_1988} introduce the \textit{active vision} paradigm.
An active observer systm is defined as a one that has some control over its sensory input.

Bajcsy~\cite{bajcsy_active_1988} defines active vision, and perception in general, as a problem of intelligent data acquisition.
...
\subsection{Visual Search in Robotics}
