...
The problem considered in this thesis is at the junction of several fields, each with their own history and recent advancements.
In this chapter, we cover background theory that is relevant for the topic of this thesis.
We also survey related work that considers problems similar to ours.

\section{Background}
\label{sec:background}

What constitutes intelligent search behavior is closely linked to how we humans search.
This has been an active area of study in neuroscience for a long time~\cite{wolfe_five_2017,eckstein_visual_2011,nakayama_situating_2011}.
The same is true for machine perception and decision-making~\cite{aloimonos_active_1988,bajcsy_active_1988}.
In recent years, deep learning has led to great improvements in the ability of machines to perceive sensory input like images~\cite{zhao_object_2019}.
Somewhere between machine perception and decision-making lies deep reinforcement learning, which is the combination of deep learning~\cite{goodfellow_deep_2016} and reinforcement learning~\cite{sutton_reinforcement_2018}.
All of these fields provide necessary and useful perspective to our considered problem.
...
While covert attention tends to be reactionary, overt attention is often more strategic.
...
Such regularities can be learned from experience and used during future searches.
...
Furthermore, if targets often co-occur with other visible elements they tend to be easier to find~\cite{eckstein_visual_2011,wolfe_five_2017}.
...
In the \textit{active vision} paradigm, the observer has some control of its sensory input~\cite{aloimonos_active_1988}.
...
One of the difficulties of active perception tasks is that they are scene and context dependent.
...
Finding an optimal solution in three-dimensions has been shown to be NP-complete, necessitating approximate solutions~\cite{andreopoulos_theory_2009,ye_complexity-level_2001}.
...
The only requirement on the function computed by each node is that it is differentiable.
...
The architecture of a neural network imposes some bias onto the learning that is expected to be useful for generalizing to unseen samples.
...
This operation can be generalized up to more than one dimension, such as 2 dimensions for images and 3 dimensions for volumes.
...
LSTM units consist of an \textit{input gate}, \textit{forget gate}, \textit{output gate} and \textit{cell}.
These are defined by the following set of equations:

\begin{align}
    i_t &= \sigma(\mathbf{w_i} [x_t, h_{t-1}]) \\
    f_t &= \sigma(\mathbf{w_f} [x_t, h_{t-1}]) \\
    o_t &= \sigma(\mathbf{w_o} [x_t, h_{t-1}]) \\
    \tilde{c} &= \tanh(\mathbf{w_c} [x_t, h_{t-1}]) \\
    c_t &= f_t \circ c_{t-1} + i_t \circ \tilde{c}_t \\
    h_t &= o_t \circ \tanh(c_t)
\end{align}

Here, \(x_t \in \mathbb{R}^d\) is the input, \(h_t \in \mathbb{R}^h\) is the \textit{hidden state} and \(c_t \in \mathbb{R}^h\) in the \textit{cell state} of the units.
The initial values are \(c_0 = 0\) and \(h_0 = 0\) and the operator \(\circ\) is the element-wise product.
The matrices \(w_i\), \(w_f\), \(w_o\) and \(w_c \in \mathbb{R}^{h \times (h+d)}\) are the weights for the input gate, forget gate, output gate, and cell respectively (with implicit biases).
The hidden state \(h_t\) is used as output of the LSTM and depends on both the input, the previous hidden state and the cell state.
Because the cell state \(c_t\) is updated using a combination of the previous cell state and a candidate cell state \(\tilde{c}_t\), 
the cell is able to remember values over arbitrarily many time steps \(t\).
...
\begin{equation}
    \mathbb{E} \left[ \sum_{k=t+1}^T \gamma^{k-t-1} r_k \right]
\end{equation}
...
Approximate solutions are more common, where the agent usually maintains an internal \textit{belief state}~\cite{kaelbling_planning_1998} which it acts on.
...
Given a Q-function, \textit{action-value} methods choose the action greedily at every state as \(\argmax_a Q_\pi(s, a)\).
...
As long as \(\pi(a|s,\boldsymbol{\theta})\) is differentiable with respect to its parameters, the parameters can be updated to optimize for the objective.~\cite{sutton_policy_1999}
...
One important aspect of RL with function approximation is that it is applicable to partially observable problems.
...
The agent has to be rewarded frequently enough to allow it to achieve its goal once.
...
In practice, rewards are typically designed through trial-and-error.
Through \textit{reward shaping}~\cite{mataric_reward_1994},
the reward is designed to guide the agent towards achieving its goal by giving additional rewards along the way.
Several iterations of a reward signal are tried until one yields expected and sufficient results.

\subsubsection{Deep Reinforcement Learning}

As mentioned in Section~\ref{sec:policy-value}, RL usually involves approximating policies and value functions.
...
In an actor-critic approach, the advantage is estimated using a learned state value function \(V(s)\) as 
...
Here, \(\mathcal{L}_\text{VF}\) is a squared-error loss between the value function approximation and the actual state value.
The coefficient \(c_1\) scales the impact of the value loss on the parameter updates.
The final term, \(S[\pi]\), denotes the entropy of the action probabilities \(\pi\).
Policies with higher entropy, or policies where action probabilities are closer to equal, can be favored by selecting a large coefficient \(c_2\).
Algorithm~\ref{alg:ppo} shows the steps of the PPO algorithm.

\begin{algorithm}
    \caption{Proximal Policy Optimization.}
    \label{alg:ppo}
    \input{algorithms/ppo}
\end{algorithm}
...
We also cover work that relates to our posed research questions.

\subsection{Search with Deep Reinforcement Learning}

There have been numerous rigorous attempts to implement robotic object search systems using non-learning methods.
...
Each object is examined from multiple perspectives and ranked by probability of being the queried object.
...
A policy for controlling a fixed pan-tilt-zoom camera is learned with reinforcement learning.
...
Applying a CNN to large images can be expensive, as the complexity scales linearly with the number of image pixels.
They introduce a recurrent model that extracts information from images by adaptively selecting a sequence of smaller regions to process at high resolution.
...
State-of-the-art object detection methods employ deep learning techniques, usually a convolutional architecture, to recognize objects~\cite{zhao_object_2019}.
...
Furthermore, the entire scene that is searched for objects is visible in the image.
In \textit{active object detection}, the analyzed images are instead chosen to help the detector perform its task better.
...
Visual navigation has been studied for use in both autonomous ground vehicles, where obstacles have to be avoided, and for unmanned aerial vehicles which typically don't have the problem of avoiding obstacles~\cite{bonin-font_visual_2008}.
Visual search can be considered analogous to a visual navigation in unrestricted environments.
...
There are multiple simulated environments for such tasks~\cite{kolve_ai2-thor_2019,xia_gibson_2018,savva_habitat_2019}.
...
A reward signal with a constant time penalty is used to incentivize task completion in few steps.
...
Their agent is also told what object to look for in the form of an encoded word.
The encoded object graph and word is used as input together with the camera image to an actor-critic network that selects actions that navigate towards the target object.
As the agent collects experiences, it also updates its prior knowledge of spatial relationships between objects.
It is shown that prior knowledge encoded in such a way improves the agent's ability to navigate to objects.
...
Many works in visual navigation investigate the use of spatial memories rather than temporal ones.
...
This architecture is shown to outperform LSTM and the architecture in \cite{oh_control_2016} on a set of navigation tasks which require remembering over many time steps.
Several similar architectures have been proposed~\cite{henriques_mapnet_2018,gupta_cognitive_2019,chaplot_object_2020}.

\subsection{Generalization and Inductive Bias}

In RL, generalization refers to an agent's ability to act well in environments that have not been seen during training.
...
When there is no specific inductive bias to encode, standard regularization can be used.
Finally, when we can not rely on specific inductive biases or standard regularization, we have to rely on learning invariances from training and hope that these generalize to testing.
...
An issue in state-of-the-art deep RL is reproducibility.
...
which in turn has led to a shift to only evaluating a few runs (\(N \leq 5\)) per task~\cite{agarwal_deep_2022}.
...
They find that hyperparameters and the choice of network architecture for policy and value function approximation can influence performance significantly.
