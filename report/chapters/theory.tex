...
Simple memory mechanisms, like some inhibition-of return~\cite{itti_koch_2001} mechanism that prevents searching visited locations twice, improve search time in random fields considerably.
...
Since attention is limited, both due to field of view and limited processing, 

Two big limitations of performance in visual search are processing power, and limited observability.
Our vision is foveated, meaning that we\dots
This is typically not true for computer vision systems.
Such restraints
...
Introduce the paradigm called \textit{active vision}~\cite{aloimonos_active_1988}, and prove that an active observer can solve several basic vision problems in a more efficient way than a passive one.
...
Mnih et al.~\cite{mnih_attention_2014} propose a recurrent model of visual attention\dots
...
% - there are differences between the tasks
% - the existing environments can not test what we are looking for
% - we can draw inspiration from several approaches
% - memory is important and interesting

% - they only test indoor scenarios
% - in reality they are too ambitious for me 
