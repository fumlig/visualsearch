...
Vision in humans is foveated - we perceive with different resolutions at different parts of the retina.
For computer vision systems, this is typically not a limitation in the sensors.
However, processing large images might be too expensive, even for computer systems.
In such cases, images have to be attended to one part at a time.

Scanning an environment for targets involves the use of \textit{visual attention}.
...
Eye movement is \textit{saccadic} - rapid and almost instant between locations.
This is in general not true for computer vision systems: cameras are more often  
...
Natural environments are in fact seldom completely random, and exhibit some structure.
When finding targets, there tends to be something that can be learned from previous experience to improve performance.
Such regularities can learned and used during future searches.
Improvements in human search performance have been measured when certain locations have higher probabilities of containing targets~\cite{eckstein_visual_2011}.
Furthermore, if targets usually co-occur with other visible elements they tend to be easier to find~\cite{eckstein_visual_2011}.

\todo{\cite{wolfe_five_2017}}
...
Aloimonos et al.~\cite{aloimonos_active_1988} introduce the \textit{active vision} paradigm.
An active vision is defined as a system that involves an observer that has some control over its sensory input.
They prove that an active observer can solve several basic vision problems in a more efficient way than a passive one.
When the transformation of the observer's view is known, it can uniquely compute shape and motion.
...
In a re-visitation of active perception, Bajcsy, Aloimonos and Tsotsos~\cite{bajcsy_revisiting_2018} stress that despite recent successes in robotics, artificial intelligence and computer vision, an intelligent agent must include active perception:
...
\end{quote}~\cite{bajcsy_revisiting_2018}

% surveillance section
Active perception in general relates to the idea of moving a sensor to constrain the interpretation of the environment.
Chen et al.\cite{chen_activevisionsurvey_2011} survey recent developments in active vision.
They refer to the task of finding a given object in a known or unknown environment as object search.
This task involves object recognition, localization, sensing control, environment modelling and path planning.
Viewpoint selection in object search is similar to that of data acquisition.
Visual matching has been shown to be NP complete, with exponential time complexity relative to the size of the image.

% tsotsos is probably a good reference
...
% Q-learning vs. Policy-G: https://flyyufelix.github.io/2017/10/12/dqn-vs-pg.html
...
DQN inspired many several follow-up works which use deep neural networks to approximate policy and value functions.
...
\todo{Several papers state this without providing a reference.}

\section{Related Work}

To our knowledge, there is no work that considers the exact problem we are looking at.
In this section we present work that considers similar tasks, focusing on RL methods.
...
A policy for controlling a fixed pan-tilt-zoom camera is learnt using reinforcement learning.
...
Despite being limited to a single environment, this is an 
...
A similar work by Shubina and Tsotsos~\cite{shubina_visualsearch_2010} consider the problem of visual search in 3D environments with a mobile robot.
They use\dots 
...
Attention has been modelled in machines with success for both language~\cite{bahdanau_attention_2016} and vision tasks.
While overt attention is hard attention, it is also interesting to take inspiration from soft attention, which more closely resemble covert attention.
...
More recently, deep RL seen use for visual navigation as well~\cite{zeng_survey_2020}.
...
They propose a new architecture, that in addition to an image, also observes the reward, velocity and the action from the previous time step.
...
Their experiments make the same assumptions as \cite{zhu_target_2016} - the scenes and targets used during testing have all been seen during training.
...
As mentioned in Section \ref{sec:pomdp}, effective behaviour in partially observable processes usually requires some form of memory.
...
For the visual search task we consider here, memory is likely to be important.
It is likely that integrating features over time helps with finding targets quicker.
...
Memory architectures that are specialized for particular tasks, like \cite{}, have the potential to be more useful and give better performance.
...
The memory contains a mechanism for retrieving memory contents based on the current context of the agent.
...
They propose a formalism for collections of problems called contextual CMDPs.
...
In a CMDP, separate train and test tasks can be defined by creating \(\mathcal{C}_\text{train} \subset \mathcal{C}\) and \(\mathcal{C}_\text{test} \subset \mathcal{C}\) such that \(\mathcal{C}_\text{train} \cap \mathcal{C}_\text{test} = \varnothing\).
They conclude that strong generalization requires 
