...
This chapter introduces relevant theory.
...
One early example is TD-Gammon~\cite{tesauro1995tdgammon}, a neural network trained with RL that reached expert Backgammon performance in the 1995.
...
DQN approximates the quality function \(q\) with a CNN, and select actions greedily using only visual input.
...
\subsection{Memory for Deep Reinforcement Learning}
...
Finally, they make the point that more emphasis should be placed on applying RL algorithms to real-world tasks.
...
They recomnend success weighted by (normalized inverse) path length (SPL) as the primary measure of navigation performance.
...
Finally, they emphasize the importance of memory mechanisms that support the construction of rich internal representations of the agent's environment.
