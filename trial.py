...
class SignalEnv(gym.Env):
    ...
    def __init__(self, signal, window, delta=None):
        ...
        self.window = window
        self.delta = delta if delta is not None else np.ones(len(self.shape), dtype=int)

        assert len(self.shape) == len(self.window)
        assert all([window_dim <= shape_dim for shape_dim, window_dim in zip(self.shape, self.window)])

        self.reward_range = (-np.inf, np.inf)
        self.action_space = gym.spaces.Discrete(len(self.shape)*2)
        self.observation_space = gym.spaces.Box(0, 1, self.window)

    def reset(self):
        self.position = np.zeros(len(self.shape), dtype=int)
